...
int	load_sprite_images(t_data *dt)
{
	...
	sprites[i].sprite_img = mlx_xpm_file_to_image(dt->mlx_ptr, "./sprites/sprite-1.xpm", &sprites[i].width, &sprites[i].height);
	sprites[i].sprite_data = (int *)mlx_get_data_addr(sprites->sprite_img, &sprites->bpp, &sprites->size_line, &sprites->endian);
