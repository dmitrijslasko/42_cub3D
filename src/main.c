...
/*   Updated: 2025/06/06 18:08:14 by fvargas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_map	*load_dummy_map(void)
{
	...
	for (size_t curr_col = 0; curr_col < map->map_size_cols; curr_col++)
	{
		...
		if (curr_col == 0 || curr_col == map->map_size_rows - 1)
			strcpy(map->map_data[curr_col], "11111111");
		else if (curr_col == 4)
			strcpy(map->map_data[curr_col], "1N000011");
		else
			strcpy(map->map_data[curr_col], "10000001");
	}
	return (map);
}
